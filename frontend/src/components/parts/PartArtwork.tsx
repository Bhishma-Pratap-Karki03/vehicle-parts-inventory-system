...
function PartArtwork({ resolvedImageUrl, title, category, className = 'aspect-[5/3]' }: PartArtworkProps) {
  if (resolvedImageUrl) {
    return (
      <div className={`overflow-hidden rounded-[24px] border border-[#D8E3EE] bg-[#F7FAFD] ${className}`}>
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[24px] border border-[#D8E3EE] bg-[radial-gradient(circle_at_24%_20%,rgba(255,255,255,0.42),transparent_22%),linear-gradient(135deg,#0F2949_0%,#22466D_42%,#A7BED5_100%)] ${className}`}
    >
      ...
      <div className="relative flex h-full flex-col justify-between p-5 sm:p-6">
        ...
        <div className="flex items-end justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-[14rem] break-words text-[20px] font-semibold leading-tight text-white [font-family:var(--font-display)]">
          ...
          <span className="inline-flex h-16 w-16 shrink-0 items-center justify-center rounded-[20px] border border-white/18 bg-white/12 text-white shadow-[0_14px_30px_rgba(10,20,34,0.2)] backdrop-blur-sm">
            <Icon name="box" className="text-[32px]" />
          </span>
        </div>
      </div>
    </div>
